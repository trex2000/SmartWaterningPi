...
redrawNeeded = True
## Longer sleep time.
T_LONG_SLEEP = 10#3600

#cyclic call of function that retrieves most recent ambient values
T_AMBIENT_LOGGING_SLEEP_VALUE = 60 # 1 minute val

#set init value of measured data
currentHum = -1
...
def main_menu():
        ...
        global font, highlighted, menuPage, selectedItem, button1PressedEvent, redrawNeeded
        ## Number of elements in the main_menu.
        numberOfElements = len(root[menuPage])  # Number of elements in the menu.
        lastElement = int(numberOfElements) - 1  # Number of indexes of the elemnt in the menu.
        if redrawNeeded:
            redrawNeeded = False
            oled.fill(0)
            image = Image.new('1', (WIDTH, HEIGHT))  
        ...
        button1PressedEvent.clear()  # The event is marked as 'not set' via this function.
        if button2PressedEvent.is_set():  # Checks if the event was fired 
            # If the last item is selected ('exit') with push button 2, the previous menu shall display.             
            if lastElement == selectedItem: 
                selectedItem = menuPage - 1  # The selected item is the one which submenu was opened.
                menuPage = 0
                redrawNeeded = True
            else:  # The OLED should display the selecetd items menu.
                #1menuPage = selectedItem + 1  
                if (root[menuPage][selectedItem].get('callable')=='true'):                    
                    textToDisplay = RunSelectedFunction(root[menuPage][selectedItem].text)
                else:
                    nextMenuItemToFind=root[menuPage][selectedItem].text
                    for j in range(0, numberOfElements):
...
def  RunSelectedFunction (argument)
        global currentHum, currentTemp
        if ((argument) == "3. Current humidity"):
            #return  get_HumidityRecord()
            return currentHum
